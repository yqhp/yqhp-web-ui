...
import { getPagingProjects, deleteProject } from "@/api/console/project"
import { ElMessageBox } from "element-plus"
import { CirclePlus } from "@element-plus/icons-vue"
import SaveProject from "./components/SaveProject.vue"
import SaveProjectPlugin from "./components/SaveProjectPlugin.vue"
import { formatDateTime } from "@/utils/date"

const saveProjectRef = ref()
const saveProjectPluginRef = ref()
...
const handleProjectPlugin = (row) => {
  saveProjectPluginRef.value.open(row)
}

const handleUpdate = (row) => {
  saveProjectRef.value.open(false, row)
}

const handleCreate = () => {
  saveProjectRef.value.open(true)
}
...
const getTableData = async () => {
  loading.value = true
  const res = await getPagingProjects(searchParam).finally(() => {
    loading.value = false
  })
...
<template>
  <div class="h-full p-5 flex flex-col" v-loading="loading">
    ...
    <div class="mb-1">
      <el-button type="primary" :icon="CirclePlus" @click="handleCreate">新增项目</el-button>
    </div>
      ...
      <el-table height="100%" :data="tableData.records">
        ...
        <el-table-column fixed="right" label="操作" width="250">
          <template #default="{ row }">
            ...
            <el-button type="primary" text bg size="small" @click="handleProjectPlugin(row)">启用插件</el-button>
    ...
    <save-project ref="saveProjectRef" @ok="getTableData" />
    <save-project-plugin ref="saveProjectPluginRef" />
  </div>
</template>
